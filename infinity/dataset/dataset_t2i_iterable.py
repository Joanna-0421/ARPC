...
class T2IIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        for _ in range(self.samples_div_gpus_workers_batchsize_2batches):
            ...
            img_id = [item[2] for item in batch_data]
            yield (images, captions, img_id)
            del batch_data
            del images
            del captions
            del img_id
    ...
    def prepare_model_input(self, data_item) -> Tuple:
        ...
        h_div_w = 1.0
        # h_div_w = 1.5 if random.random() < 0.5 else 0.666
        img_id = data_item['id']
        img_path = os.path.join("/data/qiu_workspace/zzy/Dataset/coyo/data_ocr", str(img_id)+".jpg")
        ...
        if self.load_vae_instead_of_image:
            ...
            return True, (text_input, img_B3HW, str(img_id))
...
if __name__ == '__main__':
    ...
    dataset = T2IIterableDataset(
        args=None, 
        meta_folder='/gemini/code/ICLR2026/ARPC/data/coyo_kodak',
        data_load_reso=None, 
        max_caption_len=512, 
        short_prob=1.0, 
        load_vae_instead_of_image=False,
