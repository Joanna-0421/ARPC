...
class MultiScaleBSQ(Module):
    ...
    def forward(
        ...
        with autocast('cuda', enabled = False):
            for si, (pt, ph, pw, pm) in enumerate(scale_schedule):
                ...
                if self.training and self.use_stochastic_depth and random.random() < self.drop_rate:
                    if (si == 0 and self.keep_first_quant) or (si == scale_num - 1 and self.keep_last_quant):
                        quantized, indices, _, loss = self.lfq(interpolate_residual, pm)
                    ...
                    quantized, indices, bit_indices, loss = self.lfq(interpolate_residual, pm)
...
class BSQ(Module):
    ...
    def forward(
        ...
        with quantization_context():
            ...
            if self.new_quant:
                quantized = self.quantize_new(x, pm)
