        ...
        with torch.amp.autocast('cuda', enabled = False):
            ...
            for si, (pt, ph, pw, pm) in enumerate(vae_scale_schedule):
                ...
                if si != len(vae_scale_schedule)-1:
                    residual = F.interpolate(residual, size=vae_scale_schedule[si][:3], mode=self.vae.quantizer.z_interplote_down).contiguous()
                quantized, _, bit_indices, loss = self.vae.quantizer.lfq(residual, pm) # quantized shape: [B, d_vae, 1, h, w], bit_indices shape: [B,1,h,w,d_vae]
                ...
                cum_var_input = cum_var_input + F.interpolate(quantized, size=vae_scale_schedule[-1][:3], mode=self.vae.quantizer.z_interplote_up).contiguous()
                if si < len(vae_scale_schedule)-1:
                    this_scale_input = F.interpolate(cum_var_input, size=vae_scale_schedule[si+1][:3], mode=self.vae.quantizer.z_interplote_up).contiguous()
