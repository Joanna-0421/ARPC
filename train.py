...
os.environ["WANDB_API_KEY"] = "b02f496204d9fe7750f9a49ae396043fa3ebbad7"
# 一定要在任何 torch.compile / Triton 编译动作前设置
os.environ["CC"]  = "gcc"
os.environ["CXX"] = "g++"

# 如果你之前也设置了 CUDA_HOME、LD_LIBRARY_PATH
# conda_prefix = os.environ.get("CONDA_PREFIX")
# conda_prefix = "/mnt/data/anaconda3/envs/VAR_zzy"
# if conda_prefix:
#     os.environ["CUDA_HOME"]        = conda_prefix
#     os.environ["LD_LIBRARY_PATH"] = conda_prefix + "/lib:" + os.environ.get("LD_LIBRARY_PATH", "")
#     os.environ["LIBRARY_PATH"]    = conda_prefix + "/lib:" + os.environ.get("LIBRARY_PATH", "")
import gc
import json
import math
import os
...
def main_train(args: arg_util.Args):
    ...
    # world_size = int(os.environ["WORLD_SIZE"]) #### change!!!!
...
def main():     # # 'pt_le_ft' in train_vae.py is the same as 'pt_le_ft' in train_gpt.py
    ...
    ############### simulate multi-processing #####################
    tdist.init_process_group(
        backend='nccl',
        init_method='tcp://127.0.0.1:23456',   # 这种方式也行，Torch 会把 world_size=1、rank=0 当作单机单卡
        world_size=1,
        rank=0,
    )
    ###############################################################
