...
class InfinityTrainer(object):
    ...
    def train_step(
        ...
        inp_B3HW: FTen, text_cond_tuple: Union[ITen, FTen], img_id: str, args: arg_util.Args,
        ...
        with self.gpt_opt.amp_ctx:
            ...
            x_BLC_wo_prefix, gt_ms_idx_Bl = self.bitwise_self_correction.flip_requant(scale_q, inp_B3HW, raw_features, device)
        ...
        if not math.isfinite(loss) or math.isnan(loss):
            print(f'Nan!!!!!:{img_id}')
        ...
        if metric_lg.log_every_iter or it == 0 or it in metric_lg.log_iters:
            ...
            for scale_ind in range(min(training_scales, len(scale_schedule))):#这块计算每个scale的accuracy和loss
